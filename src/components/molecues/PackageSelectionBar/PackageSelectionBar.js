...
const PackageSelectionBar = ({ items, label = 'Pakiet*', isWhite, handleInputChange, inputValue, name, register, defaultCheckedValue }) => {
  const handleInput = (e) => {
      ...
      <ItemWrapper onChange={handleInput}>
        {items.map(({ label, value }, index) => (
          <BarItem
            ...
            value={value}
            name={name}
            checkedValue={inputValue || defaultCheckedValue}
            register={register}
          >
            {label}
          </BarItem>
